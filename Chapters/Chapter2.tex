...
\chapter{Rigidity in graphs from the Stochastic viewpoint} % Main chapter title

\label{Chapter2} % For referencing the chapter elsewhere, use \ref{Chapter1} 

\lhead{Chapter 2. \emph{Rigidity in graphs from the Stochastic viewpoint}} % This is for the header on each page - perhaps a shortened title
...
Complex topological spaces arise quite natural in a lot of scientific contexts. Probability theory provides different approaches to model those spaces; even in complex configurations, it can be possible by doing approximations, to study topological invariants. Most of the times, the main goal is to study the asymptotic behavior of a property when a parameter of the model tends to infinity.
...
Stochastic topology finds its motivation in applied problems such as statistical mechanics, configuration spaces of mechanic links, robotics, animated simulations, manufacture, and so on. Nevertheless, recent articles have used its techniques to provide probabilistic analogs of very classical topology conjectures, like Whitehead conjecture \cite{Costa15}.
...
We are interested in giving formal meaning to expressions like \textit{"quite common"} or \textit{"many"}. For a great survey in studying \textit{"typical"} topological properties of random simplicial complexes we can refer to \cite[M. Khale]{surveyStochastic}.
...
We can also think this model like $\binom{n}{2}$ Bernoulli r.v. i.i.d with parameter $p$ that represent the edges. Using what we know about this r.v. we can immediately get some properties of the degree of a given vertex $v$. 
...
Having in mind the degree distribution can be helpful to do optimizations in the rigid expansions algorithms. In the next chapter we will see how exactly can this be done.
...
For the calculations of the first event take a look to the following picture
